...
**Documentation governance is complete. The next decision is to select ONE outstanding M4 stabilization item, verify its failure mechanism in current code, define a bounded reproducer and acceptance criteria, and explicitly authorize its implementation.** F8 (finalizer contract) is a **proposed candidate** because a review describes a false-PASS boundary; selecting or fixing it has not been authorized merely by merging documentation. S4/F3/F4/F5/F6/F7 remain open. Do not begin M5, reactivate LLMC, automatically synchronize `main` or remove `__noop_check__`.
